# CV Project Progress Tracker

## Phase 1: Foundations
- [X] Week 1: Python Data Stack & Regression (Due: Nov 28)
- [ ] Week 2: MNIST CNN built from scratch (Due: Dec 05)

## Phase 2: Computer Vision
- [ ] Week 3: OpenCV Preprocessing Script (Due: Dec 12)
- [ ] Week 4: Geometry & Perspective Transform (Due: Dec 19)
- [ ] Week 5: Data Collection & Labeling (Due: Dec 26)

## Phase 3: The Engine
- [ ] Week 6: Train YOLOv8 Model (Due: Jan 02)
- [ ] Week 7: Cropping Pipeline (Due: Jan 09)
- [ ] Week 8: Digit Segmentation (Due: Jan 16)
- [ ] Week 9: Recognition / Inference (Due: Jan 23)
- [ ] Week 10: Logic & Validation Layer (Due: Jan 30)

## Phase 4: Production
- [ ] Week 11: FastAPI Implementation (Due: Feb 06)
- [ ] Week 12: Dockerization (Due: Feb 13)
- [ ] Week 13: GitHub Polish & README (Due: Feb 20)
- [ ] Week 14: Final Buffer (Due: Feb 27)